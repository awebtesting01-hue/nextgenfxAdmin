...
import { activationApi } from "../../apis/activationApi";
...
export default function ActivationFee() {
  ...
  const fetchFees = async () => {
    try {
      const res = await activationApi.getAllFees();
      setFees((res.data as unknown as GetFeesResponse).fees);
    } catch (err: any) {
      alert(err.message || "Failed to fetch activation fees");
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    // Ensure fromAmount is less than toAmount (if toAmount is set)
    ...
    try {
      if (editingId) {
        const res = await activationApi.updateFee(editingId, feePayload);
        setFees(
          fees.map((fee) => (fee._id === editingId ? res.data.fee : fee))
        );
        setEditingId(null);
      } else {
        const res = await activationApi.createFee(feePayload);
        setFees([...fees, res.data.fee]);
      }
      resetForm();
    } catch (err: any) {
  ...
  const deleteFee = async (id: string) => {
    const confirmDelete = window.confirm(
      "Are you sure you want to delete this activation fee slab?"
    );

    if (!confirmDelete) return; // User clicked "No"

    try {
      await activationApi.deleteFee(id);
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              ...
              <p className="text-gray-600">
                Configure activation fee slabs for different amount ranges
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-gray-600 text-sm">
                  Create or edit activation fee slabs
                </p>
              </div>
            </div>
          </div>
                    ...
                    <td colSpan={6} className="px-6 py-12 text-center">
                      <div className="flex flex-col items-center gap-3">
                        ...
                        <p className="text-sm text-gray-400">
                          Add your first activation fee slab above
                        </p>
                      </div>
                    </td>
                  </tr>
