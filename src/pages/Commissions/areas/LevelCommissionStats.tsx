import { Database, TrendingUp, Calendar, Clock, Layers, BarChart3 } from 'lucide-react';
import { LevelCommission } from '../../../apis/levelCommissionApi';

interface LevelCommissionStatsProps {
    commissions: LevelCommission[];
}

export default function LevelCommissionStats({ commissions }: LevelCommissionStatsProps) {
    return (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6 border border-emerald-100 hover:shadow-xl transition-all duration-300">
                <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="bg-blue-100 p-2 rounded-full">
                        <Layers className="text-blue-600 text-sm" />
                    </div>
                </div>
                <div>
                    <p className="text-sm font-medium text-gray-600 mb-1">Levels Covered</p>
                    <p className="text-3xl font-bold text-gray-900">
                        {new Set(commissions.map(s => s?.level))?.size || 0}
                    </p>
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6 border border-purple-100 hover:shadow-xl transition-all duration-300">
                ...
                <div>
                    ...
                    <p className="text-3xl font-bold text-gray-900">
                        {commissions.length > 0 ? 'Today' : 'None'}
                    </p>
